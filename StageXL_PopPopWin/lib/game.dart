...
class Game {
  final Field field;
  final bot.Array2d<SquareState> _states;
  final bot.EventHandle<bot.EventArgs> _updatedEvent = new bot.EventHandle<bot.EventArgs>();
  final bot.EventHandle<GameState> _gameStateEvent = new bot.EventHandle<GameState>();
  ...
  Game(Field field) :
    ...
    _states = new bot.Array2d<SquareState>(field.width, field.height, SquareState.hidden) {
    assert(field != null);
  ...
  void setFlag(int x, int y, bool value) {
    ...
    final currentSS = _states.get(x, y);
    if (value) {
      bot.require(currentSS == SquareState.hidden);
      _states.set(x, y, SquareState.flagged);
      _bombsLeft--;
    } else {
      bot.require(currentSS == SquareState.flagged);
  ...
  List<bot.Coordinate> reveal(int x, int y) {
    _ensureStarted();
    bot.require(canReveal(x, y), "Item cannot be revealed.");
    final currentSS = _states.get(x, y);

    List<bot.Coordinate> reveals;

    // normal reveal
    if (currentSS == SquareState.hidden) {
      if (field.get(x, y)) {
        _setLost();
        reveals = <bot.Coordinate>[];
      } else {
  ...
  List<bot.Coordinate> _doChord(int x, int y) {
  ...
  void _update() => _updatedEvent.add(bot.EventArgs.empty);
